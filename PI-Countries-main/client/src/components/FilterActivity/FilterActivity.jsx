...
const FilterActivity = ({setPage}) => {
    ...
    return(
        ...
        <div className={style.filterActivity}>
            <label htmlFor="activities"> Activities: 
            <br/>
            {errorFilter && <span>No activities</span>}
            <select id="activities" onChange={(e)=> handleChange(e)} className={style.activities}>
                   <option >All</option> 
                   {activity?.map((act) => (
                   ...
                   <button onClick={(e)=> handleClick(e)} className={style.button}>X</button>
                   <br/>
                   {filters.length > 0 && filters?.map((filter)=>(
                    <span key={filter}>. {filter} <br/> 
                    </span>
                   ))}
            </label>
